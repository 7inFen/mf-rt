...
export default async function (config: AxiosRequestConfig, filename = ''): Promise<any | AxiosResponse<any>> {
  ...
  if (respData) {
    ...
    fileDownload(resBlob, extractFilenameFromResponseHeader(response) || filename)
    // 方便调用者有进一步的 then().catch()处理
